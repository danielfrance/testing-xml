...
import { Button, Table, Link, Badge, Input, Textarea } from "react-daisyui";
...
export default function ApplicantIndex({ auth, applicants, invitees }) {

    const { messages } = usePage().props

    const [creationType, setCreationType] = useState(null);
    const [showCreateApplicantModal, setShowCreateApplicantModal] = useState(false);
    ...
    const handleCreateOwnerSelection = () => {
        setShowCreateApplicantModal(false);

        if (creationType === 'manual') {
            // redirect user to owners.create
            window.location.href = '/applicants/create';
        }
        if (creationType === 'applicant') {
    ...
    const filteredApplicants = applicants?.filter(applicant => {
        return (
            ...
            applicant.fincen_id?.includes(searchTerm) ||
            (searchTerm.toLowerCase().includes("verified") && applicant.info_verified_at != null)
        );
    });
    ...
    return (
        <AuthSidebarLayout user={auth.user}>
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg px-2 py-3">
                        <div role="tablist" className="tabs tabs-lifted tabs-lg">
                            <input
                                type="radio"
                                name="applicants_tabs"
                                role="tab"
                                className="tab"
                                aria-label="All Applicants"
                                defaultChecked

                            />
                            <div className="tab-content bg-base-100 borer-base-300 rounded-box p-6">
                                <div className="flex justify-between mb-3 items-center">
                                    <h2 className="font-bold text-2xl">Company Applicants</h2>
                                    <Button
                                        color="primary"
                                        className="font-bold text-white"
                                        startIcon={<PlusIcon className="h-4 w-4" />}
                                        onClick={() => setShowCreateApplicantModal(true)}
                                    >
                                        Add Company Applicant
                                    </Button>
                                </div>
                                <div className="flex justify-between mb-3 items-center">
                                    <Input placeholder="Search Company Applicants" className="w-full" onChange={handleSearchChange}
                                        ...
                                        <Table size="lg">
                                            ...
                                            <Table.Body>
                                                {filteredApplicants.map(applicant => (
                                                    <Table.Row className="hover:bg-primary/10" key={applicant.id}>
                                                        <span>{applicant.first_name} {applicant.last_name}</span>
                                                        <span>{applicant.email}</span>
                                                        <span>
                                                            {applicant.info_verified_at ? (
                                                        ...
                                                        <span>
                                                            <Link href={route('applicants.edit', applicant.id)} className="text-primary">Edit</Link>
                                                        </span>
                                                    </Table.Row>
                                                ))}
                                            </Table.Body>
                            ...
                            <div className="tab-content bg-base-100 borer-base-300 rounded-box p-6">
                                <div className="flex justify-between mb-3 items-center">
                                    ...
                                    <h2 className="font-bold text-2xl">Invited Applicants</h2>
                                    <Button
                                        color="primary"
                                        className="font-bold text-white"
                                        startIcon={<PlusIcon className="h-4 w-4" />}
                                ...
                                <div className="flex justify-between mb-3 items-center">
                                    <Input placeholder="Search Invited Applicants" className="w-full" onChange={handleSearchInvitedChange} />
                                </div>
                                <div className="overflow-x-auto">
                                    <div className="overflow-x-auto">
                                        <Table size="lg">
            ...
            <Modal show={showCreateApplicantModal} maxWidth="2xl" closeable={true} onClose={() => setShowCreateApplicantModal(false)}>
                <div className="bg-white overflow-hidden p-6">
                    ...
                    <h3 className="mt-3 text-gray-900 text-lg mb-3">
                        Start by selecting how a new applicant will be created:
                    </h3>
                    <div className="grid grid-cols-2 gap-4 mt-6">
                        <div className="border border-gray-200 rounded-lg p-4 text-center content-center">
                            ...
                            <p className="text-gray-700">
                                You will enter the Applicants's Info.
                            </p>
                            <Button
                        ...
                        <div className="border border-gray-200 rounded-lg p-4 text-center content-center">
                            <h4 className="font-bold text-gray-900 text-lg">
                                Invite Company Applicant
                            </h4>
                            <p className="text-gray-700">
                                Send to the Company Applicant to fill out.
                            </p>
                            <Button
                                onClick={() => setCreationType('applicant')}
                                className={`btn text-white font-bold mt-4 ${creationType === 'applicant' ? 'btn-primary' : 'btn-neutral'}`}>
                                Select
                            </Button>
                        </div>
                    </div>
            ...
            {showInviteOwnerModal && (
                <InviteOwnerModal
                    title={"Invite Company Applicant"}
